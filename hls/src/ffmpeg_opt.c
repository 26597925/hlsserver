...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
	...
	InputStream *ist = input_streams[source_index];
	InputFile *inputFile = input_files[ist->file_index];
	
	if ((strstr(inputFile->ctx->iformat->name, "flv") != NULL) || 
			(strstr(inputFile->ctx->iformat->name, "mp4") != NULL) || 
			(strstr(inputFile->ctx->iformat->name, "mov") != NULL) ||
			(strstr(inputFile->ctx->iformat->name, "3gp") != NULL))    
	{
		if(ist->dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO){
			ost->bitstream_filters = av_bitstream_filter_init("h264_mp4toannexb");
		}
		if(ist->dec_ctx->codec_type == AVMEDIA_TYPE_AUDIO){
			ost->bitstream_filters =  av_bitstream_filter_init("aac_adtstoasc");  
		}
	}
