                         ...
                         double sync_ipts)
{
    ...
    if (!next_picture) {
		printf("!next_picture:=================================%d\n", ost->sync_opts);
        //end, flushing
        nb0_frames = nb_frames = mid_pred(ost->last_nb0_frames[0],
                                          ost->last_nb0_frames[1],
                                          ost->last_nb0_frames[2]);
    } else {
		printf("video_sync_method:=================================%d\n", video_sync_method);
        ...
        case VSYNC_PASSTHROUGH:
            ost->sync_opts = lrint(sync_ipts);
			printf("VSYNC_PASSTHROUGH:%d=================================\n", ost->sync_opts);
            break;
        default:
            av_assert0(0);
        }
    }
	printf("do_video_out sync_opts6:%d addr :%x=================================\n", ost->sync_opts,&ost);

	printf("yinwb->sync_opts:%d=================================\n", ost->sync_opts);
