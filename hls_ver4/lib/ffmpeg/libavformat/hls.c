...
static int hls_read_header(AVFormatContext *s)
{
    ...
    for (i = 0; i < c->n_playlists; i++) {
		...
		if (strstr(s->filename, "file/m3u8:fd")){
			pls->ctx->probesize = 50000000;
		}
        else
        {
            if(s->duration > 20) // discriminate VOD stream 
            {
                pls->ctx->probesize = 4*1024*1024;
                pls->ctx->max_analyze_duration = 10*1000*1000;
            }
            else    //discriminate living stream
            {
               pls->ctx->probesize = 200*1024; // prevent video information err of width or height
               pls->ctx->max_analyze_duration = 4*1000*1000;
            }
        }
